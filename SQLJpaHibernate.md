...
1. B-дерево (B-tree)
   - Поддерживает операции: =, >, <, BETWEEN, LIKE 'pattern%'
     Каждый узел B-дерева содержит:
     - Ключи (k₁, k₂, ..., kₙ), упорядоченные по возрастанию.
     - Указатели на дочерние узлы (p₀, p₁, ..., pₙ), если узел не листовой.
     - Флаг листа (является ли узел терминальным).
   Вставка элемента идет так: тут используются ноды и вычисляется ключ который сравнивается с объектом подлиста и если он меньше то идет слева если больше то справа и обходим до тех пор пока ключи не равны.
2. Хеш-индекс
