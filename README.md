...
Уровни изоляции транзакций определены в виде констант интерфейса Connection:
1. TRANSACTION_NONE
...
2. managed (Persistent) — объект создан, управляется JPA, имеет сгенерированные первичные ключи. В это состояние может попасть объект из 
   абсолютно любого состояния. Что бы перевести объекты из состояния Transient в Persistent - надо воспользоваться методом persist. Для 
...
Отличие от DELETE:
Скорость (TRUNCATE — мгновенно, DELETE — построчно).
Нет журналирования (нельзя ROLLBACK в некоторых СУБД).
