...
IF(логическое_выражение, выражение_1, выражение_2) <br/>
округления числа до знаков после запятой: Round(число, 2) as new_item <br/>
HAVING - используется для условий с GROUP BY  `SELECT author, MIN(price) AS Минимальная_цена FROM book GROUP BY author HAVING SUM(price * amount) > 5000 ORDER BY Минимальная_цена DESC;` <br/>
UNION и UNION ALL — это операторы SQL, которые объединяют результаты двух и более запросов в одну таблицу. <br/>
<br/>
