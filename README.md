...
1. **Инкапсуляция** - скрывает детали реализации объекта внутри, предоставляя только необходимы методы для использования.
2. **Наследование** - добавляет возможность создавать новые классы на основе существующих, добавляя в них новую необходимую логику.
...
Используется когда необходимо: 1. Определение API для внешних модулей. 2. Функциональные интерфейсы для лямбд. 3. несколько реализаций одного контракта.
Интерфейс не может иметь реализации, поля только (public, static), можно реализовать много интерфейсов, есть дефолтные методы, нету конструктора.

**абстрактный класс** — описывает как он это делает, частично реализуя логику. <br>
Используется когда необходима общая реализация, необходимость состояние полей, частичная реализация.
можно создавать конструктор, любые поля, класс может только один абстрактный класс наследовать

**IoC (Inversion of Control)** — это принцип передачи управления созданием объектов контейнеру. Вместо ручного внедрения зависимостей, 
фреймворк берет это на себя посредством контейнера. Реализуется через Dependency Injection (внедрение зависимостей).
...
**Процесс** -  это экземпляр программы со своей памятью и ресурсами, изолированный от других процессов. Внутри которой работают потоки. <br>
**Поток** - это часть процесса, которая разделяет его память, но выполняется независимо. Тоесть внутри процесса у нас есть потоки.

## DeadLock_LiveLock_Starvation

**DeadLock** - ситуация когда 2 и более потока блокируют друг друга и ждут освобождения ресурсов которые сами и заблокировали.
Он возникает при выполнении 4 условий: взаимное исключение, удержание и ожидание, отсутствие вытеснения и круговая зависимость
Предотвратить можно: 1. Упорядочивание блокировок. 2. Использование таймаутов. 3. Использование ExecutorService, ConcurrentHashMap

**Livelock** - это ситуация, когда потоки активно выполняют работу, но система не продвигается из-за циклических зависимостей в их действиях
