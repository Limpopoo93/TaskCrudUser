...
1. **Инкапсуляция** - скрывает детали реализации объекта внутри, предоставляя только необходимы public методы для использования.<br>
3. **Наследование** - добавляет возможность создавать новые классы на основе существующих, добавляя в них новую необходимую логику.<br>
5. **Полиморфизм** - позволяет объектам с одинаковым интерфейсом иметь различную реализацию.<br>

**Ассоциация** - объекты двух классов могут ссылаться один на другой, имея связь друг с другом.(Типы: один к одному, Многие ко многим) <br>
**агрегация** - ассоциация со слабой связью. односторонняя связь с одноправленным отношением. (Пример: двигатель и автомобиль) <br>
**композиция** - это ассоциация с сильной связью. Когда класс владеет объектом и несет ответственность за его время жизни. (сотрудник и роль)

**Интерфейс** — описывает что класс должен делать. <br>
Используется когда необходимо: 1. Определение API для внешних модулей. <br>
2. Функциональные интерфейсы для лямбд. 3. несколько реализаций одного контракта.<br>
Интерфейс не может иметь реализации, поля только (public, static), можно <br>
реализовать много интерфейсов, есть дефолтные методы, нету конструктора.

**абстрактный класс** — описывает как он это делает, частично реализуя логику. <br>
Используется когда необходима общая реализация, необходимость состояние полей, <br>
частичная реализация. можно создавать конструктор, любые поля, класс может только <br>
один абстрактный класс наследовать

**IoC (Inversion of Control)** — это принцип передачи управления созданием объектов контейнеру. Вместо ручного внедрения зависимостей, фреймворк берет это на себя <br>
посредством контейнера. Реализуется через Dependency Injection (внедрение зависимостей).<br>
**DI** — это частный случай IoC, при котором зависимости объекта <br>
(сервисы, репозитории)не создаются внутри класса, а внедряются извне <br>
(через конструктор, сеттер или поле).
...
3. **System Classloader** - загружает классы из переменной окружения CLASSPATH.<br>
   Кроме того, в Java можно создавать пользовательские загрузчики классов, которые<br>
   могут загружать классы из любых других источников, например, из базы данных или из сети.

## Мусор_JVM
Типы сборщиков мусора:
1. **Serial** - Использует один поток.
2. **Parallel** - Использует несколько потоков. (Работа со средними и большими наборами данных.)
3. **G1** - Выполняет некоторую тяжелую работу параллельно с работой приложения.<br>
   (Куча делится на регионы одинакового размера,и эти регионы могут принадлежать<br>
   либо молодому поколению, либо старому.)
5. **Z1** - Выполняет всю тяжелую работу параллельно с работой приложения. Низкая<br>
   задержка. Управляемая задержка через параметр.<br>
6. **ZGC** - новое решение в котором минимальная задержка (ZGC использует так <br>
   называемые load barriers - Это высоко оптимизированный код, так что потоки <br>
   приложения на Java будут работать практически незаметно, получая доступ к объекту)
...
4. Консистентность - повторные вызовы метода equals() для одного объекта должны <br>
   возвращать одинаковый результат

## Generics
**Generics** — это механизм параметризации типов, который позволяет создавать <br>
  обобщённые классы, методы, интерфейсы. Generics в Java реализованы через <br>
  стирание типов (Type Erasure). <br> На этапе компиляции информация о generics <br>
  удаляется, и в байт-коде остаются только сырые типы <br>
? extends Number   (можем только читать) <br>
? extends T        (можем только писать) <br>
...
**Лямбда-выражения** — это анонимные функции, которые позволяют кратко и удобно <br>
реализовывать методы интерфейсов с одним абстрактным методом (функциональных интерфейсов).
...
**Comparable** — это интерфейс для естественного порядка сортировки int compareTo(T other). Реализуется самим классом, объекты которого нужно сравнивать. <br>
например Integer — сортируется по числовому значению<br>
Используется в Collections.sort(list)<br>
**Comparator** — Интерфейс для внешней сортировки. Позволяет создать много разных способов сравнения одного и того же класса. int compare(T o1, T o2)<br>
например, сортировка карточки по типу или Collections.sort(list, byName);<br>
Реализуется в отдельном классе/лямбде через метод compare(T o1, T o2)<br>
...
2. **LinkedHashSet**: Хранит уникальные элементы с порядком их добавления. <br>
   Высокопроизводительная. Удаление дубликатов без потери порядка.<br>
   add в среднем O(1), в худшем при коллизиях 0(n).<br>
   remove\contain\size: O(1).<br>
3. **SortedSet**: Реализацией является TreeSet. Элементы отсортированы. <br>
   Предоставляет сортировку по естественном порядку (Comparable) или с помощью Comporator.<br>
  add/remove/contains: O(log n)<br>
  first/last: O(1)<br>
  методы: first/last/headSet(E elem) возвр.подмножество элем.строго меньше <br>
  elem/tailSet(E elem) возвр.подмножество элем.больше или равно <br>
  elem/subSet(E fromE, toE)возвр.подмножество в диапазоне/comparator<br>
4. **TreeSet**: Элементы отсортированы с использованием самобалансирующего <br>
   красно-черного дерева. Используется для элементов в отсорт. порядке согласно <br>
   естественном порядку comparable или компоратору, Коллекция используется когда <br>
   нужны диапазоны значений(headSet, tailSet), часто требуется first, last элем. <br>
   Запрет на null значения.<br>
  add/remove/contains O(log n)<br>
  first/last/size/isEmpty O(1)<br>
5. **IdentityHashSet**: сравнение элементов по == а не по equals(). порядок не <br>
   гарантируется. Производительность как и у hashmap.

1. **PriorityQueue**: это очередь с приоритетами, которая хранит элементы в <br>
   отсортированном порядке(по умолчанию — натуральный порядок или компаратор)<br>
   а не в порядке добавления.<br>
   null элементы запрещены.<br> Использует Comparator или естественный порядок (Comparable).
   <br> Основана на двоичной мин-куче— это дерево, где каждый родитель меньше своих потомков.<br>
   offer/add/poll - O(log n)<br>
   remove/contains - O(n) (contains (поиск) - Поиск в куче требует полного обхода, <br>
   так как нет гарантии порядка элементов (кроме корня))<br>
   peek/size - O(1)<br>
...
3. **Deque**: Добавление/удаление с обоих концов (головы и хвоста). Разрешены Null элементы.<br>
   ...
   все операции: О(1)<br>
4. **ArrayDeque**: реализация Deque на основе массива. <br>Автоматическое расширение,<br>
   может работать как стек и очередь одновременно.<br> Не поддерживает null.<br>
...
3. **TreeMap**: реализована как красно-чёрное дерево, которая хранит элементы в <br>
   отсортированном порядке сс использованием красно-черного дерева, что позволяет эффективно работать с диапазонами.<br>
...
**Процесс** -  это экземпляр программы со своей памятью и ресурсами, изолированный <br>
  от других процессов. Внутри которой работают потоки. <br>
**Поток** - это часть процесса, которая разделяет его память, но выполняется независимо. <br>
  Тоесть внутри процесса у нас есть потоки.
...
**DeadLock** - возникает, когда два или более потока бесконечно ожидают ресурсы, заблокированные друг другом.<br>
Он возникает при выполнении 4 условий: взаимное исключение, удержание и ожидание, <br>
отсутствие вытеснения и круговая зависимость<br>
Способы решения:<br>
1. Упорядочивание блокировок правильно захватывать Lock потока
2. Используйте tryLock() с таймаутом вместо бесконечного ожидания
3. Используйте атомарные операции

**Livelock** - это ситуация, когда потоки активно выполняют работу, но система не<br>
продвигается из-за циклических зависимостей в их действиях<br>
Например, два потока бесконечно уступают друг другу ресурс, как вежливые люди в коридоре.
Избежать можо: добавление thread.sleep(), ограничение попыток выполнения в циклах<br>
Способы решения:<br>
1. Введение случайных задержек
2. Ограничение числа попыток
3. Приоритизация потоков

**Starvation(голодание потока)** - результат процесса который не может получить <br>
регулярный доступ к ресурсам необходимым для выполнения задачи, и соответственно <br>
не может добиться какого либо прогресса.

## Что_такое_happens_before_в_Java_Memory_Model

**Happens-before** — это формальное отношение между операциями в многопоточной среде,<br>
которое гарантирует, что одна операция видит результаты другой операции.
...
1. **CountDownLatch** - Позволяет одному или нескольким потокам ждать, пока другие<br>
   потоки не завершат определенные операции Инициализируется числом (count), <br>
    которое уменьшается методом countDown(). Потоки, вызывающие await(),<br>
    блокируются, пока счетчик не станет 0
2. **CyclicBarrier** - Заставляет набор потоков ждать друг друга в определенной <br>
   точке (барьере), прежде чем продолжить выполнение.<br>
...
**acid:**
1. Атомарнось - Транзакция выполняется как единое целое — либо полностью, либо не выполняется вообще
2. Согласованность - Транзакция переводит базу данных из одного согласованного состояния в другое
   Если в рамках транзакции создавались индексы, ключи и т.д. то в случае отката транзакции они должны тоже удалиться)
4. Изолированность - Параллельные транзакции не влияют друг на друга
5. Долговечность - Если транзакция зафиксирована, ее результаты сохраняются даже при сбое системы
...
2. **TRANSACTION_READ_UNCOMMITTED** – транзакция может видеть результаты других транзакций, даже если они ещё не закоммичены.<br>
...
3. **TRANSACTION_READ_COMMITTED** – транзакция может читать только те изменения в других параллельных транзакциях,
   которые уже были закоммичены.<br>
...
4. **TRANSACTION_REPEATABLE_READ** – пока транзакция не завершится, никто параллельно не может изменять или удалять строки,
   которые транзакция уже прочитала.<br>
...
**Отличие стартера от библиотеки:** <br>
Starter автоматически настраивает бины и компоненты, в то время как обычная библиотека требует ручной конфигурации.<br>
Starter включает все необходимые транзитивные зависимости с гарантированно совместимыми версиями, тогда с обычной библиотекой разработчик<br>
	 должен самостоятельно подбирать совместимые версии зависимостей.<br>

**@Configuration** — это базовая аннотация Spring для определения конфигурационных классов, содержащих бины. Ручное определение бинов в любом Spring-приложении<br>
**@AutoConfiguration** — специализированный вариант для механизма автоконфигурации Spring Boot. Автоматическая настройка функциональности в Spring Boot Starter'ах<br>
	регистрация через специальный файл, а не component scanning<br>

**почему у стартеров autoconfiguration** <br>
Позволяет starter'ам "магически" добавлять функциональность. Приложение получает готовую функциональность без ручной конфигурации.  не нужно писать одинаковые конфиги в каждом проекте<br>

## Kafka

**Алгоритмы зжатия кафки**:
1. GZIP
2. Snappy
3. LZ4

**Есть два типа Consumer**:
1. **assign** - явно говорим которые TopicPartition будем читать
2. **subscribe** - можно сказать что хотим читать из какогото множества топиков

**Два типа обработки сообщений**:
1. **Queue** - каждое сообщение обрабатывается одним consumer в группе.
2. **Publish-Subscribe** - все сообщения обрабатываются всеми consumers.

Есть две гарантии обработки:
1. **at least once**.(commit до обработки) - минимум один раз.<br>
   Сообщение гарантированно доставляется, но возможны дубли<br>
   Для критически важных данных (платежи, транзакции)<br>
   Для настройки: Настройки подтверждения (acks), Повторные попытки
3. **At most once**.(commit после обработки) - максимум один раз.<br>
   Сообщение может быть потеряно, но никогда не будет доставлено повторно<br>
   Для данных, где потеря допустима (например, метрики, логи)<br>
   Для настройки: отключить подтверждение(acks=0) и отключить повторные отправки (retries=0).
4. **Exactly once** (ровно один раз) - сообщение будет доставлено и обработано один раз.<br>
   Сообщение обрабатывается строго один раз<br>
   Финансовые операции, где дубли недопустимы<br>
   Для обеспечения: Идемпотентный Producer
...
## Паттерны
1. **Принцип единственной ответственности (Single Responsibility Principle, SRP)** <br>- класс должен иметь только одну
   ответственность. <br> Например, класс, отвечающий за работу с базой данных, не должен также заниматься обработкой
   пользовательского ввода или выводом на экран.<br>
...
2. **Принцип открытости/закрытости (Open/Closed Principle, OCP)** <br> - классы должны быть открыты для расширения, но закрыты для
   модификации. <br> Это означает, что новый функционал должен добавляться через добавление новых классов или методов,
   а не изменение существующих.
3. **Принцип подстановки Барбары Лисков (Liskov Substitution Principle, LSP)** <br> - класс который наследовался от родителя не должен <br>
   ломать интерфейс. <br> Допустим если мы наследовались от класс который не бросает Exception а я бросаю Exception то <br>
   родительский класс может этого не знать и у него будет не объяснимое поведение. Тоесть мы должны написать код так <br>
   чтобы при использовании в одном месте класса и его подкласса и при этом не будет не предвиденных ситуаций. <br>
   К примеру у нас есть класс прямоугольник и у него есть два поля ширина и высота. Мы создаем класс Квадрат который наследуется <br>
...
5. **Принцип инверсии зависимостей (Dependency Inversion Principle, DIP)** <br> - класс должен зависеть не от реализации других классов
   а от абстракций тоесть допустим его интерфейса. Главная цель DIP — уменьшить связанность (coupling) между компонентами системы. <br>
   Ваш код должен зависеть от интерфейсов (что нужно сделать), а не от классов (как это сделано).  <br>
   Это позволяет: <br>
   - Легко тестировать компоненты (например, подменять реальную базу данных на заглушку). <br>
   - Легко изменять реализацию, не ломая основной код. <br>
   - Создавать модульную и масштабируемую архитектуру. <br>
...
**Порождающие:**
1. **Абстрактная фабрика** - представляет собой интерфейс для создания других классов, позволяя создавать семейства связанных объектов не привязываясь к конкретным классам. <br>
    Паттерн используется когда: <br>
   - Необходимо создавать объекты которые используются вместе, к примеру элементы графического дизайна(кнопки,флажки,меню) <br>
   - Система должна поддерживать несколько конфигураций к примеру для windows and macOS <br>
   `interface Button { <br>
       void paint(); <br>
       void onClick(); <br>
   }` <br>
   `class WindowsButton implements Button { <br>
       @Override <br>
       public void paint() { <br>
          System.out.println("Painting a Windows-style button."); <br>
       } <br>
       @Override <br>
       public void onClick() { <br>
          System.out.println("Windows button clicked."); <br>
       } <br>
   }` <br>
   `class MacOSButton implements Button { <br>
       @Override <br>
       public void paint() {
...
3. **Строитель (Builder)** -  - позволяет создавать сложные объекты пошагово. Для примера StringBuilder Или в Spring аннотация @Builder().  <br>
   Паттерн используется для: <br>
   - Когда объект требует много необязательных полей, и не все из них требуются каждый раз. <br>
   - Процесс строительства объекта разделен на этапы которые выполняются в нужном порядке. <br>
   Паттерн используется к примеру при создании HttpRequest.(не все поля нужны). <br>
4. **Фабричный метод (Factory method)** -Определяет интерфейс для создания объекта в суперклассе, но позволяет подклассам изменять <br>
   ти объектов которые будут созданы. Метод позволяет создавать различные объекты без жесткой привязки кода к конкретным классам. <br>
   Паттерн используется когда: <br>
   - Точный тип создаваемого объекта не известен до момента выполнения. <br>
   - Логика объектов сложна, повторяется или требует инкапсуляции. <br>
   - Необходимость следовать принципу открытости\закрытости. <br>
...
5. **Прототип (Prototype)** - Определяет интерфейс создания объекта через клонирование другого объекта вместо создания через
   конструктор. <br>
6. **Одиночка (Singleton)**

**Структурные:**

1. **Адаптер** - Позволяет несовместимым интерфейсам работать вместе, преобразовывая интерфейс одного класса в другой. <br>
   Паттерн используется когда: <br>
   - Выполняется интеграция с устаревший системой или сторонней библиотекой которая не соответствует желаемому. <br>
   - Необходимо повторно использовать существующую функциональность не изменяя ее исходный код. <br>
   interface Car(){} <br>
...
2. **Мост (Bridge)** - позволяет разделить абстракцию и реализацию так, чтобы они могли развиваться независимо, не влияя друг на друга напрямую. <br>
   Есть интерфейс с методом, мы его через конструктор можем передать в абстрактный класс без реализации методов а<br>
   ...
   интерфейса.<br>
4. **Компоновщик (Composite)** - это паттерн, который позволяет сгруппировать объекты в дерево и затем работать с ними как с одним объектом.  <br>
    Используется к примеру для реализации файловой системы (иерархии папок). <br>
    ...
    class BigContainer implements Container { <br>
     ...
     <br>
6. **Декоратор (Decorator)** - Динамически добавляет новую функциональность объекту, не меняя основной класс. (когда класс реализует интерфейс, имплементит
   его метод и добавляет дополнительные методы)
7. **Фасад (Facade)** - Предоставляет упрощенный интерфейс к сложной системе. (тоесть это реализован метод в котором находятся множество взаимодействий с дочерними классами их вызов и логика)
...
8. **Приспособленец (Flyweight)** - позволяет эффективно поддерживать множество объектов, разделяя их общее состояние. <br> Тоесть в классе есть метод который служит как фабрикой для создания классов. <br>
...
4. **Стратегия (Strategy)** - позволяет определить набор алгоритмов, поместить каждый из них в отдельный класс и сделать их объекты взаимозаменяемыми  <br>
   что позволяет алгоритму изменяться независимо от того кто его использует. Используется для множественной замены if-else, или switch
    Паттерн позволяет изменять поведение объекта на лету.  <br>
...
1. **Быстрая сортировка**: использует стратегию разделяй и властвую.
   ...
   - Пространственная сложность: O(log n) (из-за рекурсии)  <br>
   Берется элемент (pivot), может быть первым\последним, средним, или случайным элементом.  <br>
   Массив разделяется на две части, левая часть элементы меньше выбранного, правая часть больше.  <br>
   Дальше происходит рекурсивная сортировка левой и правой части массива.  <br>
2. **Сортировка слиянием**: использует стратегию разделяй и властвую.  <br>
   Массив рекурсивно разбивается на две половины пока не останутся подмассивы из одного элемента. <br> После этого массивы с одним элементом у каждого объединяются в один упорядоченный
   массив.  <br> Хорошо работает для больших данных и внешней сортировки (например, сортировка файлов)
   - лучший\худший\средний - O(n log n)
   - Пространственная сложность: O(n) (требуется дополнительная память)
3. **Сортировка вставкой**: строит итоговый массив по одному элементу за раз вставляя новый элемент в правильную позицию отсортированной части.  <br>
   Разделяет массив на два подмассива (отсортированного и не отсортированного).  <br> Последовательно берем элемент из не отсортированной части и вставляем в позицию отсортированной части, сдвигая остальные.
   - Лучший случай когда массив отсортирован O(n)
   - средний\худший  O(n²)
   - Пространственная сложность: O(1) (in-place, не требует доп. памяти)
4. **Пирамидальная сортировка**: основан на структуре двоичная куча.  <br>
   Преобразует массив в max-кучу где каждый родитель больше своих потомков. Корень это макс.элемент.  <br>
   - лучший\средний\худший: O(n log n)
   - Пространственная сложность: O(1)
5. **Сортировка выбором**:
   Массив делится на две части.  <br> Одна из частей отсортирована (изначально пустая) другая не отсортирована.  <br> На каждом шаге выбирает минимальный элемент из неотсортированной части и помещает его в конец отсортированной.
...
6. **Пузырьковая сортировка**:
   ...
   - лучший O(n)
   - средний\худший O(n²)
7. **Сортировка Шелла**: улучшенная версия сортировкой вставками только сортирует элементы в интервале  <br>
...
8. **TeamSort**: комбинирует MergeSort (слияние) и InsertionSort (вставками)  <br>
   Массив разделяется на небольшие упорядоченные подмассивы (обычно длиной от 32 до 64 элементов).  <br> Каждый подмассив сортируется вставками. <br>
...
1. **Обход в глубину(DFS)** - один из популярных способов это рекурсия.  <br> Чтобы не переполнять стек можно использовать итерационный подход с Stack коллекцией. <br> Необходим для поиск пути, циклы в графе, компоненты.  <br>
...
2. **Обход в ширину(BFS)** - применяется итерационный подход чтобы избежать переполнение стека,  используются очередь Queue.  <br> Необходим для поиска кратчайшего пути, обхода по уровням, поиска в соц.сетях.  <br>
...
Отличие:  <br>
Logstash заменен на Fluentd в роли сборщика и агрегатора логов.  <br>
Основная причина появления этой альтернативы — желание использовать более легковесный и эффективный по   <br>
ресурсам инструмент для сбора и обработки данных с высокими нагрузками допустим в Kubernetes.  <br>
...
1. **ELK**  <br>
   ...
   - **Kibana**  <br>
   **Стек:**  <br>
   - **Beats** — "Сборщики данных" (Агенты) - устанавливается на сервера откуда необходимо собирать инфу. Отправляют данные
     в ElasticSearch через Logstash.  <br>
     Разновидности:  <br>
     1. **Filebeat** - Собирает данные из лог-файлов.  <br>
     2. **Metricbeat** - Собирает метрики системы (загрузка CPU, память, диск)  <br>
     3. **Packetbeat** - Собирает сетевые данные, анализирует трафик (например, HTTP-запросы)  <br>
   - **Logstash** — "Конвейер обработки данных" (Серверная обработка) - принимает данные допустим из Beats, преобразует в
     необходимый формат и отправляет их в ElasticSearch.   <br>
     Он состоиз из:  <br>
     1. **Input** - принимает входные данные  <br>
     2. **Filter** - фильтрует и изменяет данные. Он состоит из:  <br>
        - Grok - парсинг логов в необходимый нам формат, к примеру извелечения из логов IP, даты, уровня логиварония  <br>
        - Mutate - переименование, изменение, удаление полей  <br>
        - GeoIP - добавление информации по IP адресу  <br>
        - Date - парсинг даты и времени  <br>
     3. **Output** - отдает данные на ElasticSearch  <br>
   - **Elasticsearch **— "Сердце системы" (Хранилище и поиск) -  Это NoSQL база данных, которая хранит все данные в виде документов JSON  <br>
     1. Документы (Documents): Основная единица информации (например, одна строка из лог-файла — это один документ).  <br>
     2. Индексы (Indices): Коллекции документов (например, отдельные индексы для логов приложения, системных логов и метрик).  <br>
     3. Распределенность и отказоустойчивость: Данные автоматически распределяются и реплицируются между узлами кластера.  <br>
     4. Мощный полнотекстовый поиск: Очень быстрый и гибкий поиск по данным.  <br>
     5. RESTful API: Управление и запросы к данным происходят через простой HTTP API.  <br>
   - **Kibana** — "Визуализация и управление" (Веб-интерфейс) - веб-интерфейс для визуализации и управления данными из Elasticsearch  <br>
     1. Discover: Просмотр сырых данных, поиск по ним с помощью фильтров и запросов.  <br>
     2. Visualize: Создание графиков, диаграмм, таблиц и дашбордов на основе данных из Elasticsearch.  <br>
     3. Dashboard: Компоновка нескольких визуализаций на одном экране для общего обзора.  <br>
     4. Machine Learning: Поиск аномалий в данных (например, необычный всплеск ошибок).  <br>
     5. Stack Management: Управление настройками Elasticsearch и Kibana (индексы, пользователи, права доступа и т.д.).  <br>

2. **EFK**  <br>
   - **Elasticsearch** — система для хранения и поиска данных.  <br>
   - **Fluentd** — агрегатор логов, собирающий, обрабатывающий и пересылающий их в Elasticsearch.  <br>
   - **Kibana** — инструмент для визуализации данных из Elasticsearch.  <br>
   Стек:  <br>
   - **Fluentd / Fluent Bit** — "Сборщик и маршрутизатор логов" - собирает логи в оркестраторе к примеру Kubernetes  <br>
     Принцип работы:  <br>
     - на каждой ноде разворачивается Fluent Bit и он собирает логи со всех подов в текущем namespace.  <br>
     - Так же разворачивается Fluentd который собирает со всех Fluent Bit логи и отправляет в ElasticSearch  <br>
